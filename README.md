...
가칭 **모아**로 만든 실행 가능한 프로토타입입니다. 도시·장소 중심 구매자 홈, 여행 동선 중심 여행자 홈, 같은 장소의 부탁 일괄 수락, 금액 분해, 결제 보관 상태, 구매 인증, 귀국 후 국내 전달, 수령 확인과 정산을 연결합니다. 근처 여행자와 이동 시간은 예시이며 실제 반경 2km의 사람을 조회한 결과가 아닙니다. React Native 앱이 NestJS API를 호출하고 서버가 데이터를 저장합니다.

카카오·Google·네이버 OAuth 코드 흐름은 구현되어 있으며 공급자 콘솔 키를 설정해야 실제 계정 버튼이 활성화됩니다. PG와 실상품 수집 시스템은 아직 연결하지 않았습니다. 장소 상세와 직거래 위치는 Google 지도를 사용합니다. 국내 장소 검색은 별도 카카오 로컬 API 키가 필요합니다. 앱에서 체험 모드로 표시하며, 실제 자금 이동이나 상품 구매는 발생하지 않습니다. `NODE_ENV=production`에서는 서버가 시작되지 않습니다. 운영 서비스를 완성했다고 주장하는 버전이 아닙니다.
...
4. **모아 시작하기 → 체험 계정으로 로그인**을 누릅니다. 로그인 뒤 **MY → 이용 모드 → 부탁하기 / 여행하기**로 모드를 바꿀 수 있습니다.
...
- 구매자 홈은 여행 도시·방문 장소를 중심으로 보여주며 링크·사진 부탁은 작은 빠른 시작 영역에서 시작합니다. 여행자 홈은 내 일정의 장소별 부탁과 등록된 보상을 보여줍니다.
- 하단 `+`는 부탁·여행 중 시작할 일을 고르는 시트를 엽니다. 등록·결제 단계에서는 하단 탐색 탭을 숨겨 현재 작업에 집중합니다. 이용 모드 변경은 MY·설정에 모았습니다.
- 여행지는 국가·도시·섬을 검색하고 선택합니다. 국가만 고를 수도 있으며 방문 예정지·날짜를 선택한 뒤 왕복 항공권 대조로 이어집니다.
- MY는 프로필·인증·거래 지표와 짧은 그룹 메뉴로 구성했습니다. 배송지 관리, 완료한 직거래 장소 기록, 결제수단, 정산 계좌, 알림 설정, FAQ를 제공합니다. 실제 외부 서비스가 연결되지 않은 메뉴는 준비 상태와 사용할 수 있는 다음 행동을 설명합니다.
- 공통 시트·검색창·탭과 타이포그래피·여백·색상 토큰으로 화면의 일관성을 맞춥니다. 설계 근거는 [디자인 시스템](docs/02-design-system.md), 변경 목록은 [제품 감사](docs/06-product-audit.md)에 있습니다.

서비스 이해 **5초**, 부탁 등록 **30초**는 목표이며 사용자 테스트로 측정한 결과가 아닙니다. 자동 테스트 101개, 타입·lint·빌드·DOM 흐름과 별도 Chrome의 360/390/548px 검사를 통과했습니다. 외부 장소 검색은 모의 응답을 사용했으며 실제 지도·검색 공급자 및 네이티브 실기기 검증과 구분합니다. 상세 범위는 [검증 기록](docs/05-validation.md)에 기록했습니다.

### 이번에 연결한 장소·재매칭 흐름

- 여행 목적지는 국가·지역 → 도시·섬 → 방문 예정지 순서입니다. 일본(도쿄·삿포로·오키나와 본섬·이시가키섬 등), 한국, 대만, 홍콩, 중국, 태국, 베트남, 싱가포르, 말레이시아, 인도네시아의 주요 여행지를 지원합니다. 한 국가 안에서 여러 도시·섬을 여행 일정에 담거나 국가만 선택할 수 있습니다. 카탈로그는 유한한 예시 목록이며 실시간 재고·방문자 수가 아닙니다.
- 현지 통화 10종을 지원합니다. 환율은 `packages/domain/src/destinations.ts`의 고정 데모 값입니다. 신규 부탁은 구매자가 보상금 `requestedReward`를 원화로 정하고 여행자가 그 금액을 확인해 수락합니다. 보상이 없는 이전 부탁만 여행자 입력으로 호환합니다. 기존 PostgreSQL 사용자는 `npm run db:migrate`로 버전 2를 적용해주세요. 원본 JSON 거래/정산 내역은 유지합니다.
...
국내 장소 검색을 사용하려면 `apps/api/.env`에 `KAKAO_REST_API_KEY`를 설정하고 서버를 다시 실행하세요. 키를 채팅이나 모바일 코드에 넣지 마세요. 앱은 검색 설정 여부를 먼저 확인하고, 미연결이면 안내와 지도 위치 지정을 제공합니다. 공급자가 실패해도 예시 결과로 대체하지 않습니다. 검색어는 카카오로 전송됩니다. [카카오 로컬 API 문서](https://developers.kakao.com/docs/latest/ko/local/dev-guide)를 참고하세요. 지도 이동으로 직접 위치를 지정하는 기능은 검색 키 없이도 사용할 수 있으며 타일을 읽지 못하면 재시도 안내를 표시합니다. 위치 변경 중 취소하면 이전 확정 위치를 유지합니다.

직거래 위치는 Google Maps JavaScript API 키가 없을 때 Kakao Map 웹 지도를 실제 지도 미리보기로 사용한다. Google 소비자 지도 URL은 iframe 차단 헤더를 반환하고, 원본 OSM 타일 직접 호출은 403을 반환할 수 있어 사용하지 않는다. 지도를 드래그·클릭해 정확한 좌표를 저장하려면 `apps/mobile/.env`에 `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY`를 설정하고 Maps JavaScript API를 활성화하세요. 장소 검색 결과로 선택한 좌표는 키 없이도 저장할 수 있다. 공개 클라이언트 키이므로 Google Cloud Console에서 허용 웹 출처를 제한해야 한다. 자동화 지도 테스트에서는 외부 지도를 모의 응답으로 대체한다.

둘러보기와 장소 상세는 실제 Google 지도를 표시합니다. 키가 없을 때는 선택한 한 장소를 Google Maps Embed로 보여주고, 다중 마커와 마커 선택을 사용하려면 `apps/mobile/.env`에 `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY`를 설정합니다. 브라우저에 포함되는 공개 클라이언트 키이므로 Google Cloud Console에서 Maps JavaScript API를 활성화하고 허용 웹 출처를 제한해야 합니다.
...
| 기능                | 이 버전의 실제 동작                                                                       |
| ------------------- | ----------------------------------------------------------------------------------------- |
| 두 역할             | 한 계정에서 `마이 → 설정`으로 부탁하기/여행하기 모드를 명확하게 전환                        |
...
| 여행 등록           | 출발지 확인, 국가·도시·섬 검색과 다중 선택, 방문 예정지, 달력, 왕복 항공권 인식·일정 대조  |
| 묶음 수락           | 같은 장소·일정 조건의 요청 선택 → 건별 거래·채팅방 원자적 생성                            |
| 구매자 지정 보상    | 구매자가 신규 부탁의 보상금을 정하고 여행자가 수락 · 체험 한도 0~200만원                 |
| 가격                | 서버 계산, 상품가+등록 보상+귀국 후 국내 전달비. 구매자 플랫폼 수수료 없음                  |
| 결제                | 카드 등록 또는 계좌 확인 후 Mock 승인·보관·중복 방지·구매 전 환불                          |
| 구매 결과           | 상품 사진 또는 영수증 1장 이상으로 인증 · 품절/휴무/구매 제한은 방문 증빙 후 전액 환불    |
| 거래 진행           | 명령별 권한/상태/버전 검증, 현재 단계·다음 행동·대화                                     |
...
| MY                  | 프로필·인증·활동 그룹, 배송지·직거래 기록·결제·정산·알림 시트, 설정 안 체험 계정 전환     |
| 지도                | 둘러보기·장소 상세와 직거래는 Google 지도. 키 연결 시 직거래 지도 이동·확대·좌표 저장 지원 |
...
4. 카드 또는 계좌를 선택해 체험용 결제수단을 확인하고 금액·운송 방식을 검토한 뒤 모의 결제합니다. ‘결제 실패 상태도 체험하기’도 가능합니다.
5. **MY → 이용 모드 → 체험 계정 전환**을 펼쳐 민트로드 계정으로 바꾸고 해당 거래를 다시 엽니다.
...
8. 같은 설정 메뉴에서 소운 계정으로 돌아와 해당 거래의 **수령하고 구매 확정하기**를 누릅니다.
...
- **MY → 이용 모드 → 여행하기**를 선택한 뒤 홈의 방문 장소 묶음에서 요청을 선택하고 수락합니다.
- 실제 구매는 구매자가 선택하고 Mock 결제까지 완료한 뒤 가능하도록 상태를 제한합니다.
- 각 부탁을 수락하면 구매자별 거래와 채팅방이 즉시 열립니다. 묶음 전체를 한 사람의 결제로 합치지 않습니다.

MY의 **설정 → 체험 계정 전환**을 펼쳐 다른 가상 사용자로 전환할 수 있습니다. 이것은 데모 전용 기능이며 이용 모드 변경·실계정 연결·관리자 권한과 다릅니다.
...
- 여행자 역할은 체험용 본인인증을 먼저 완료해야 부탁을 수락할 수 있습니다. 입력한 이름·생년월일·휴대폰 번호는 저장하지 않고 인증 결과만 기록합니다. 실서비스 본인확인 공급자는 아직 미연결입니다.
- 바코드를 읽지 못하면 선택 동의와 서버 AI 키가 있을 때만 사진 문자 인식을 시도합니다. **인식은 발권 진위·실제 예약 유효성·본인 인증이 아닙니다.** 항공사 확인 공급자는 미연결이며 새 일정은 최종 인증 전 수락이 차단됩니다. 기존 가상 인증 일정만 거래 체험에 사용할 수 있습니다.
...
| `apps/mobile/src/components/ui.tsx`      | 버튼·입력·검색·탭·시트·알림·빈 상태·페이지 등         |
| `apps/mobile/src/components/visuals.tsx` | 상품 예시 그림, 장소 카드, 타임라인, 가격           |
| `apps/mobile/src/components/RouteMap*`   | Google 장소 지도, 다중 마커, 장소 선택              |
...
npm run test:ui
npm run build
# 추가 환경: Python 3, Playwright·requests, Google Chrome
...
`npm run test:browser`는 빌드 후 별도 임시 API·정적 서버와 새 Chrome 세션에서 360/390/548px 화면을 검사합니다. 사용자 데이터와 열린 브라우저는 변경하지 않습니다. 직거래 장소 선택·저장·취소·새로고침, 요청 뒤로가기, 여행지·방문지·달력, MY·거래의 넘침과 런타임 오류를 확인합니다. 장소 검색은 명시적 테스트 결과로, 외부 지도·폰트는 테스트 응답으로 대체하므로 실제 외부 연동 검증은 아닙니다. 캡처 위치는 실행 결과에 출력됩니다.
