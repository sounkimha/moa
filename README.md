...
- 현지 통화 10종을 지원합니다. 환율은 `packages/domain/src/destinations.ts`의 고정 데모 값이며, 여행자가 부탁별 보상금을 원화로 제시합니다. 기존 PostgreSQL 사용자는 `npm run db:migrate`로 버전 2를 적용해주세요. 원본 JSON 거래/정산 내역은 유지합니다.
...
| 장소 탐색           | 아시아 국가·지역 → 도시 필터, 장소/상품/경로 검색, 관심 장소 저장                          |
...
4. 금액·운송 방식·카드 또는 보관함을 선택하고 모의 결제합니다. ‘결제 실패 상태도 체험하기’도 가능합니다.
...
- 홈 **링크 붙여넣기 → 예시 링크로 빠르게 채우기 → 수령 방법 정하기 → 부탁 등록하기**.
