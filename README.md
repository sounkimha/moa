...
- 현지 통화 10종을 지원합니다. 데모에서는 화면·결제 금액을 안정적으로 비교할 수 있도록 `1 JPY = 9.4 KRW`를 사용합니다. 서버에 `FX_RATE_MODE=LIVE`를 설정하면 최신 제공 환율을 사용하고, 결제 시 적용된 환율은 이후 거래에 고정합니다. 기본 LIVE 공급자는 키 없는 Frankfurter 일 단위 기준 환율이며, `CURRENCYAPI_KEY`를 함께 설정하면 해당 계정의 요금제에 따라 더 자주 갱신되는 환율을 사용할 수 있습니다. 공급자 실패 시에는 고정 체험 환율로 안전하게 돌아갑니다. 여행자 보상은 원화로 제시합니다. 기존 PostgreSQL 사용자는 `npm run db:migrate`로 버전 2를 적용해주세요. 원본 JSON 거래/정산 내역은 유지합니다.
