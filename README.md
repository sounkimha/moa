...
키·환경변수 없이 실행하면 `apps/api/.data/moa.json`에 저장됩니다. **체험 계정으로 로그인 → 로그인하기**에서 아이디와 비밀번호를 입력합니다. 로그인하거나 계정을 바꿔도 기존 일정·거래·가입 계정은 초기화하지 않습니다. 로그인 세션은 서버 재시작 시 만료되지만 영구 저장소의 가입 계정으로 다시 로그인할 수 있습니다.
...
테스트 비밀번호는 공유받은 체험용 값을 사용합니다. 공개 환경에서는 `MOA_TEST_PASSWORD_SHA256`, `MOA_TRAVELER_TEST_PASSWORD_SHA256`으로 바꿀 수 있습니다. 실제 사용자의 비밀번호를 테스트 계정에 사용하지 마세요.

로그인 화면의 **회원가입**에서 아이디(3~32자), 닉네임(2~24자), 비밀번호·확인을 입력하면 새 계정을 저장하고 로그인합니다. 비밀번호는 8~128자에 영문·숫자·특수문자를 포함해야 합니다. 아이디는 대소문자를 구분하지 않으며 중복·예약 아이디는 사용할 수 없습니다. 비밀번호는 서버의 비공개 `authIdentities`에 [scrypt](https://nodejs.org/api/crypto.html#cryptoscryptpassword-salt-keylen-options-callback)와 임의 salt로 저장하며 앱 데이터 응답에는 포함하지 않습니다. 기존 JSON/JSONB 저장 구조를 사용하므로 테이블 추가는 없습니다.

신규 가입 시 자동 로그인/생체 인증은 자동 활성화하지 않습니다. 다음 로그인에서 직접 선택할 수 있습니다. 가입만으로 본인인증·일정인증 배지나 실제 잔액을 부여하지 않으며, 체험 카드와 빈 포인트 보관함만 준비합니다. 실제 운영용 이메일·휴대폰 인증, 계정 복구/탈퇴, 영속 세션, 개인정보 정책은 별도 준비가 필요합니다. 영구 DB 또는 볼륨이 없는 배포는 재배포 시 계정 데이터가 사라질 수 있습니다.
...
`MOA_SERVE_WEB=1`인 체험 서버는 같은 도메인의 `/admin`에서도 콘솔을 제공합니다. 이 경로는 관리자 세션과 2단계 인증을 통과해야 하며, 일반 사용자 계정으로는 열 수 없습니다.
...
- 생체 로그인은 기기에 Face ID 또는 지문이 등록되어 있을 때 로그인 화면에서 ‘자동 로그인’과 ‘생체 인증으로 빠르게 로그인’을 함께 선택해 설정합니다. 다음 실행에는 기기 인증으로 보호된 로그인 정보만 읽고, 인증을 취소하면 아이디 로그인으로 돌아갑니다. iPhone의 Face ID는 Expo Go에서 지원되지 않으므로 위 iOS 개발 빌드를 실제 기기에 설치해 시험해야 합니다. 현재 체험 서버 세션이 만료되거나 서버가 재시작되면 아이디로 다시 로그인해야 합니다.
