        ...
        with sync_playwright() as p:
            ...
            try:
                for width,height in [(360,780),(390,844),(430,932),(412,915),(1440,960)]:
                    ...
                    expect(button(page,'구매 요청 등록')).to_be_visible()
                    inspect(page,'create-sheet-'+str(width),output)
                    button(page,'여행 일정 등록').click()
                    expect(page.get_by_text('어디로 떠나세요?',exact=True)).to_be_visible()
