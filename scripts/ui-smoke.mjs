...
try {
  ...
  await click('등록', 'tab');
  await expectText('어떤 걸 등록할까요?');
  await click('여행 등록하기');
  finishAnimations();
  await expectText('여행지는 어디인가요?');
  assert.ok(!find('홈', 'tab'), 'Focused registration hides unrelated bottom navigation');
  await click('여행지 선택 열기');
  await click('일본 전역', 'radio');
  await click('일본 전역으로 설정');
  for (const element of document.querySelectorAll('div')) {
    const animation = dom.window.getComputedStyle(element).animationName;
    if (animation && animation !== 'none') element.dispatchEvent(new dom.window.Event('animationend', { bubbles: true }));
  }
  await expectText('일본 전역');
  await click('여행지 선택 열기');
  await fill(await wait(() => document.querySelector('input[aria-label="국가 도시 섬 검색"]'), 'destination search'), '다낭');
  await expectText('베트남');
  await click('다낭 선택', 'checkbox');
  await click('1곳 선택 완료');
  for (const element of document.querySelectorAll('div')) {
    const animation = dom.window.getComputedStyle(element).animationName;
    if (animation && animation !== 'none') element.dispatchEvent(new dom.window.Event('animationend', { bubbles: true }));
  }
  await expectText('다낭');
  await click('여행지 선택 열기');
  await click('일본 보기', 'tab');
  await click('도쿄 선택', 'checkbox');
  await click('이시가키섬 선택', 'checkbox');
  await click('2곳 선택 완료');
  finishAnimations();
  await click('방문 예정지 선택');
  assert.equal(document.querySelectorAll('[role="checkbox"][aria-checked="true"]').length, 0, 'No stop is silently added');
  await click('시부야 PARCO 방문', 'checkbox');
  await click('도쿄역 캐릭터 스트리트 방문', 'checkbox');
  await click('유글레나 몰 방문', 'checkbox');
  await click('3곳을 일정에 저장');
  finishAnimations();
  await click('여행지 선택 열기');
  await click('오사카 선택', 'checkbox');
  await click('3곳 선택 완료');
  finishAnimations();
  await click('방문 예정지 선택');
  assert.equal(document.querySelectorAll('[role="checkbox"][aria-checked="true"]').length, 3, 'Retained areas preserve all chosen stops: ' + document.body.textContent.slice(-2400));
  await click('닫기');
  finishAnimations();
  await click('여행 날짜 선택');
  await expectText('가는 날부터 차례로 선택해주세요.');
  const choices = [...document.querySelectorAll('[role="button"][aria-label$=" 선택"]')].filter((element) => /^\d{4}-\d{2}-\d{2} 선택$/.test(element.getAttribute('aria-label')) && element.getAttribute('aria-disabled') !== 'true');
  assert.ok(choices.length >= 2, 'Date range exposes selectable days');
  await click(choices.at(-2).getAttribute('aria-label'));
  await expectText('오는 날을 선택해주세요.');
  await click('닫기');
  finishAnimations();
  const storedTrip = JSON.parse(dom.window.sessionStorage.getItem('moa-trip-draft:u-me'));
  assert.equal(storedTrip.places.length, 2);
  assert.deepEqual(storedTrip.customStops, ['이시가키섬 · 유글레나 몰']);
  assert.ok(storedTrip.end, 'Cancelling date selection does not erase the saved return date');
  await click('뒤로');
  await expectText('링크나 사진만');
  await click('등록', 'tab');
  await click('여행 등록하기');
  finishAnimations();
  await expectText('유글레나 몰');
  await click('뒤로');
  console.log('PASS: + chooser → country/city/island selection → retained stops → date cancellation → restored trip draft');
  await click('둘러보기', 'tab');
