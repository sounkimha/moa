...
| 거래     | 내 부탁 / 받는 거래 / 가져오는 거래 | 선결제, 지원, 여행자 선택, 진행현황, 채팅, 구매 인증, 수령 확인 | 상태와 다음 행동 중심        |
...
flowchart TD
  ...
  V --> B["내 일정에 맞는 장소별 요청"]
  B --> C["묶음에서 처리할 요청 선택"]
  C --> D["선결제된 조건·일정 확인 후 지원"]
  D --> E["구매자가 나를 선택하면 매칭·채팅 시작"]
  E --> F{"매장 구매 결과"}
...
- 동일 `placeId` + 여행 국가 + 실제 방문 장소 + 지원 가능한 상태 + 수령 기한 충족 요청을 묶는다. 한 여행에는 같은 국가의 여러 도시를 등록할 수 있다.
- 수량·중량은 여행자가 등록한 최대 처리 수량 안에서 선택한다. 현장 재고·매장별 구매 제한은 확인 필요로 표시한다.
- 서로 다른 구매자의 주문을 하나의 결제로 합치지 않는다. 일괄 지원은 확정이 아니며, 각 구매자가 한 명을 선택할 때만 거래와 채팅방을 만든다.
...
귀국 후 국내 택배는 3,500원, 직접 전달은 0원이다. 신규 부탁은 구매자가 원화 보상금 `requestedReward`를 정하고 선결제하며, 여행자는 그 금액을 확인해 지원한다. 보상은 상품가의 10%로 자동 산정하지 않는다. 체험 범위는 원 단위 0~200만원이며, 묶음에도 각 부탁의 등록 보상을 적용한다. 보상이 없는 이전 미결제 부탁은 결제 화면에 0원으로 표시하고 선결제 시 확정한다. 이미 결제한 금액은 여행자가 바꿀 수 없고, 매칭할 때 다시 결제하지 않는다.
