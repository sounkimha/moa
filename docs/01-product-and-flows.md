...
**MOA**는 이미 예정된 방문과 그곳의 구매 수요를 모은다. 수수료 정책과 상표·도메인 확보 여부는 출시 전 별도로 검증한다.
...
귀국 후 국내 택배는 3,500원, 직접 전달은 0원이다. 여행자가 부탁별 보상금을 직접 입력한다. 금액은 상품가에 연동하지 않으며 체험에서는 원 단위 0~200만원을 입력할 수 있다. 묶음에서도 요청별 금액을 각각 저장하고, 구매자가 그 보상을 포함한 총액을 확인한 뒤 결제한다. 제안 전에는 보상을 미정으로 표시하고 상품·전달비만 안내한다. 국가 간 이동은 여행자의 원래 동선이므로 별도 국제배송비를 더하지 않는다. 구매자에게 플랫폼 수수료를 부과하지 않으며, 여행자 보상의 10%를 거래별로 반올림하여 정산 시 운영 수수료로 공제한다. 묶음의 예상 순보상도 각 거래의 공제액을 합산한다. 이 공제는 여행자 수락·정산 화면에서만 표시한다. 세금 예치액은 데모에서 0원이며 실운영 세액 확정이 아니다. 기존 제안·결제·정산 금액은 이 정책 변경으로 다시 계산하지 않는다.
...
온보딩은 두 역할의 차이를 짧게 설명하되 역할 선택을 로그인 조건으로 만들지 않는다. 먼저 로그인한 뒤 홈에서 부탁할게요/가져올게요를 언제든 전환한다. 구매자 홈은 검색창보다 인기 상품 사진과 원화 환산가를 먼저 보여주고, 한 번의 탭으로 미리 채워진 요청 폼에 진입한다. 여행자 홈은 예정 경로에 가까워지면 동선 추가 시간과 요청 수를 먼저 표시하고, 부탁마다 원하는 보상금을 제안한다. 위치 기반 알림은 실제 위치 권한과 거리 계산이 연결되기 전까지 체험용 예시임을 밝힌다. 요청 폼은 입력 방법→상품 확인→수령 방식과 주소→등록의 짧은 흐름이다. 제안 비교는 도착일/보상/인증/후기를 함께 보여준다. 진행화면은 돈의 상태, 현재 단계, 다음 행동을 첫 화면에 둔다. 구매 후에는 상품 사진 또는 영수증 중 하나 이상으로 인증한다. 품절·휴무·구매 제한이면 환불 전에 구매자와 다른 매장 방문 여부를 상의할 수 있다. 환불을 선택하면 방문 증빙과 사유를 기록하고 결제금을 전액 환불하며, 구매자는 같은 상품을 다시 부탁할 수 있다.

여행자 후보에서는 **이 사람의 일정 보기**를 통해 서울→목적지 국제 이동, 현지 방문 장소 Point/Route Line, 날짜·시간 목록을 함께 제공한다. 비행기 Motion은 약 1.5초이며 정보를 가리지 않고 Reduced Motion에서는 정적 위치로 대체한다. 경로는 여행자가 공개한 계획으로 표시하며 실제 항공편·GPS 추적처럼 표현하지 않는다.

결제수단은 Mock 카드·간편결제·MOA 포인트 보관함으로 분리한다. 수령과 구매 확정 뒤 상품 선지출 상환액과 순보상을 여행자 보관함에 적립하고, 본인확인→정산계좌→출금의 Mock 흐름으로 연결한다. 카드번호 전체·계좌번호 전체를 저장하지 않으며 실제 PG/PASS/은행 Provider가 연결되지 않았음을 모든 금전 화면에 표시한다.
