...
`/bundles/offers`와 `/bundles/claim`에서는 위 내용에 `requestIds: ["r-1", "r-2"]`를 추가한다. `reward`는 구버전 클라이언트 호환을 위해 받을 수 있지만 서버가 무시하고 각 요청의 수량 반영 상품 원화 환산액 10%로 보상을 계산한다. `rewards` 같은 요청별 임의 금액표는 받지 않는다. 하나라도 상태·일정·장소·수량·금액 검증에 실패하면 전체가 rollback된다. 여러 구매자의 결제를 합치지 않는다.
