...
const { quote, recommendedReward, TRANSPORT_LABEL } = require('@moa/domain');
...
test('only return-home delivery is offered; the 12,000 shipping estimate cannot recur', () => {
  ...
  const reward = recommendedReward(request);
  assert.equal(reward, 2275);
  assert.equal(quote(request, reward, 'DOMESTIC_PARCEL').totalPrice, 28523);
  assert.equal(quote(request, reward, 'MEETUP').totalPrice, 25023);
...
function legacyDatabase() {
  ...
  const t = {
    ...
    ...quote(r, recommendedReward(r), 'DOMESTIC_PARCEL'),
  };
  t.totalPrice += 8500;
  t.shippingFee = 12000;
