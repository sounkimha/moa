...
function Dashboard({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <>
    ...
    <section className="panel period-transactions"><div className="panel-head"><div><p className="eyebrow">TRANSACTION VOLUME</p><h2>기간별 거래 현황</h2></div><span className="subtle">결제 기록 기준</span></div><div className="period-transaction-grid">{Object.values(d.periodTransactions).map((metric) => <div className="period-transaction" key={metric.label}><div className="period-transaction-head"><strong>{metric.label}</strong><span>{metric.periodLabel}</span></div><div className="period-transaction-values"><div><span>거래액</span><strong>{won(metric.transactionAmount)}</strong></div><div><span>거래 수</span><strong>{metric.transactionCount.toLocaleString('ko-KR')}건</strong></div></div></div>)}</div><p className="footnote period-footnote">한국 시간 기준의 모의 결제 기록입니다. 환불된 결제는 제외하며, 금액 조회 권한이 없는 계정에는 거래액을 노출하지 않습니다.</p></section>
